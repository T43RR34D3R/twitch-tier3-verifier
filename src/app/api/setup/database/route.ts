...
// Only allow super admins to run database setup
const SUPER_ADMIN_IDS = ['1239758967', '269187200']; // TearReader, BuckFoozle

export async function POST(request: NextRequest) {
  try {
    const token = await getToken({ req: request });
    
    if (!token || !SUPER_ADMIN_IDS.includes(token.sub || '')) {
    ...
    return NextResponse.json(
      { error: 'Failed to setup database: ' + (error as Error).message },
      { status: 500 }
    );
  }
}
